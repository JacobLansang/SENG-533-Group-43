...
time_query(pet_collection, lambda: pet_collection.find({ "petID": 7777 }))
...
result = pet_collection.aggregate([
    {
        "$match": { "PetID": 7777 }  # Match documents with the desired PetID
    },
    {
        "$lookup": {
...
owners_collection.delete_one({"OwnerID": 2001})

time_query(pet_collection, lambda: pet_collection.insert_one({
    "PetID": 10001
}), "ins")
pet_collection.delete_one({"PetID": 10001})
...
time_query(owners_collection, lambda: owners_collection.update_one({"OwnerID": 5000}, { "$set": {"Surname": "SMITH"}}), "update")
owners_collection.update_one({"OwnerID": 5000}, { "$set": {"Surname": "BRADLEY"}})

time_query(pet_collection, lambda: pet_collection.update_one({"PetID": 10000}, { "$set": {"Name": "Finnegan"}}), "update")
owners_collection.update_one({"PetID": 10000}, { "$set": {"Name": "Gretel"}})